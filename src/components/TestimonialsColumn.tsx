          ...
          ...new Array(2).fill(0).map((_, index) => (
            <React.Fragment key={index}>
              {props.testimonials.map(({ text, image, name, role }, i) => (
                <div className="p-6 sm:p-10 rounded-3xl border border-gray-200 shadow-lg shadow-green-500/10 max-w-xs w-full bg-white" key={i}>
                  <div className="text-gray-700 text-sm sm:text-base">{text}</div>
